					...
					<?foreach($arResult['SECTIONS'] as $SID => $arSection):?>
						...
						<?endif;?>
								...
								<?foreach($arSection['ITEMS'] as $i => $arItem):?>
									...
									$imagePath = false;
									if ($imageDetailSrc) {
										$imagePath = $imageDetailSrc;
									} else {
										if ($bImage) {
											$imagePath = $arItem['PREVIEW_PICTURE']['SRC'];
										}
									}
										...
										<?if($bImage || $onlyImgPart):?>
											<div class="image <?=($bImage ? ' w-picture' : ' wo-picture wpi')?>">
												...
												<?elseif(isset($arItem['FIELDS']['DETAIL_PICTURE'])):?>
													<a href="<?=isset($imageDetailSrc) ? $imageDetailSrc : $imageSrc;?>" alt="<?=($bImage ? $arItem['PREVIEW_PICTURE']['ALT'] : $arItem['NAME'])?>" title="<?=($bImage ? $arItem['PREVIEW_PICTURE']['TITLE'] : $arItem['NAME'])?>" data-caption="<?=$arItem['NAME'];?>" class="img-inside fancy shine">
												<?endif;?>
											...
											<div class="item colored_theme_hover_bg-block <?=($bImage ? '' : ' wti')?> box-shadow bordered clearfix " id="<?=$this->GetEditAreaId($arItem['ID'])?>">
												...
												<div class="body-info">
													...
													<?if(isset($arItem['FIELDS']['DETAIL_PICTURE'])):?>
														<?if($imagePath):?>
															<a href="<?=$imagePath;?>" data-caption="<?=$arItem['NAME'];?>" class="fancy zoom_wrap colored_theme_hover_bg-el bordered rounded3 muted ">
